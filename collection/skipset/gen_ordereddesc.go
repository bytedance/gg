...
type OrderedSetDesc[T constraints.Ordered] struct {
	length       int64
	highestLevel uint64 // highest level for now
	header       *orderednodeDesc[T]
}

type orderednodeDesc[T constraints.Ordered] struct {
	flags bitflag
	value T
...
func newOrderedNodeDesc[T constraints.Ordered](value T, level int) *orderednodeDesc[T] {
	n := &orderednodeDesc[T]{
		value: value,
		level: uint32(level),
	}
...
func unlockorderedDesc[T constraints.Ordered](preds [maxLevel]*orderednodeDesc[T], highestLevel int) {
...
// ToSlice converts this skipset into a slice.
func (s *OrderedSetDesc[T]) ToSlice() []T {
