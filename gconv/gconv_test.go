...
func testToNumber[T constraints.Number](t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf(tt.name, gvalue.Zero[T]()), func(t *testing.T) {
			if got := To[T](tt.input); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("To() = %v, want %v", got, tt.want)
			}
...
func testToUnsupported(t *testing.T) {
	tests := []struct {
		name    string
		input   any
		want    bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := To[bool](tt.input); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("To() = %v, want %v", got, tt.want)
			}

			wantPtr := gcond.If(tt.wantErr, nil, &tt.want)
			if got := ToPtr[bool](tt.input); !reflect.DeepEqual(got, wantPtr) {
				t.Errorf("ToPtr() = %v, want %v", got, wantPtr)
			}

			wantR := gcond.If(tt.wantErr, gresult.Err[bool](errUnsupported), gresult.OK(tt.want))
			if got := ToR[bool](tt.input); !reflect.DeepEqual(got, wantR) {
				t.Errorf("ToR() = %v, want %v", got, wantR)
			}

			got, err := ToE[bool](tt.input)
			if (err != nil) != tt.wantErr {
				t.Errorf("ToE() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
